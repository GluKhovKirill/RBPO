...
def uid_generator():
    ...
    for data in all_data:
        url_code = base64.b64encode((f"{data[0]}_4").encode("UTF-8"))
        final_code = str(url_code).split("'")[1].strip("==")
        username = data[4]
        mail = data[5]
        qr_fname = qr_maker(final_code)
        mess = f"""
{data[2]} {data[3]}, 22 апреля пройдет четвёртый день цикла лекций по фундаментальным информационным технологиям, их развитию в России и в мире «Школа фундаментальных технологий РБПО»
...
📍 Где? 
Читальный зал младших курсов МГТУ им. Н.Э. Баумана (Главный учебный корпус, 345 аудитория), 2-я Бауманская улица, 5с3 (ниже приложены схемы прохода)

📍 Программа дня: 🔃ИНТЕРПРЕТАТОРЫ🔃

️️➡️«Java VM - внутренний мир виртуальной машины, проблемы JIT компиляции и сборки мусора, организация процесса безопасной разработки ПО». 
⏲14:00 - 16:00
👥Спикер: 👤Александр Сергеевич Дроздов, инженер по РБПО и информационной безопасности "Axiom JDK".

☕️Кофе-брейк: 16:00-16:30

️️➡️«Python - история создания, основные идеи и механизмы». 
⏲16:30 - 18:30
👥Спикеры: 👤Никита Андреевич Соболев, Независимый open-source разработчик
