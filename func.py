...
def uid_generator():
    ...
    for data in all_data:
        print(data)
        url_code = base64.b64encode((f"{data[0]}_3").encode("UTF-8"))
        final_code = str(url_code).split("'")[1].strip("==")
        username = data[4]
        mail = data[5]
        qr_fname = qr_maker(final_code)
        mess = f"""
{data[2]} {data[3]}, 11 апреля пройдет третий день цикла лекций по фундаментальным информационным технологиям, их развитию в России и в мире «Школа фундаментальных технологий РБПО»
...
️️➡️«Контейнеризация и виртуализация - вчера, сегодня, завтра». 
⏲14:00 - 16:00
👥Спикер: 👤Александр Дубинин, эксперт по информационной безопасности "YADRO".

☕️Кофе-брейк: 16:00-16:30

️️➡️«Построение высоконагруженных сред с применением виртуальной инфраструктуры». 
⏲16:30 - 18:30
👥Спикеры: 👤Александр Иванин, руководитель отдела архитектурных решений ООО "Базис"

👤Натали Дуботолкова, старший инженер по РБПО ООО "Базис"
