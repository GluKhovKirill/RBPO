...
def uid_generator():
    ...
    for data in all_data:
        url_code = base64.b64encode((f"{data[0]}_5").encode("UTF-8"))
        final_code = str(url_code).split("'")[1].strip("==")
        username = data[4]
        mail = data[5]
        qr_fname = qr_maker(final_code)
        mess = f"""
{data[2]} {data[3]}, 27 мая пройдет пятый день цикла лекций по фундаментальным информационным технологиям, их развитию в России и в мире «Школа фундаментальных технологий РБПО»
...
📍 Программа дня: 🔎Комопозиционный и компонентный анализ🔍

️️➡️«Безопасное использование Open Source». 
⏲11:30 - 13:30
👥Спикер: 👤Алексей Смирнов, генеральный директор ООО "Профископ", "CodeScoring".

☕️Кофе-брейк: 13:30-14:00

️️➡️«Безопасность инфраструктур под управлением оркестра Kubernetes». 
⏲14:00 - 16:00
👥Спикер: 👤Дмитрий Евдокимов, генеральный директор "Luntry"
...
"""
        ...
        while True:
            try:
                ...
                if is_successful:
                    logging.info(f"sent 2 {mail}")
                    qr_flag_changer(username)
                    break
                else:
                    logging.warning(f"err sent 2 {mail}")
